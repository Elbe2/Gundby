...
bool CBitmap::Init(ID3D11Device *device,int screenwidth,int screenheight,WCHAR *filename,int bitmapwidth,int bitmapheight)
{
...
int CBitmap::GetIndexCount(void)
{
	return m_IndexCount;
}

bool CBitmap::InitBuffers(ID3D11Device *device)
{
	VertexType *vertices;
...
void CBitmap::ShutdownBuffers(void)
{
	if(m_IndexBuffer)
	{
...
bool CBitmap::UpdateBuffers(ID3D11DeviceContext *devicecontext,int x,int y)
{
	float left;
	float right;
	float top;
