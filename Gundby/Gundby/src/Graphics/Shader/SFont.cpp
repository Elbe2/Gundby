...
#include "src\Graphics\Shader\SFont.h"
...
bool CSFont::Init(ID3D11Device *device,HWND hWnd)
{
	if(!InitShader(device,hWnd,L"Shaders/font/font.vs",L"Shaders/font/font.ps"))
		return false;
	return true;
}

void CSFont::Shutdown(void)
{
	ShutdownShader();
}

bool CSFont::Render(ID3D11DeviceContext *devicecontext,int indexCount,XMMATRIX worldmat,XMMATRIX viewmat,XMMATRIX projmat,ID3D11ShaderResourceView **texture,XMFLOAT4 pixelColor)
{
	if(!SetShaderParameters(devicecontext,worldmat,viewmat,projmat,texture,pixelColor))
		return false;
	RenderShader(devicecontext,indexCount);
	return true;
}

bool CSFont::InitShader(ID3D11Device *device,HWND hWnd,WCHAR *vsFilename,WCHAR *psFilename)
{
...
void CSFont::ShutdownShader(void)
{
	if(m_SampleState)
	{
...
bool CSFont::SetShaderParameters(ID3D11DeviceContext *devicecontext,XMMATRIX worldmat,XMMATRIX viewmat,XMMATRIX projmat,ID3D11ShaderResourceView **texture,XMFLOAT4 pixelColor)
{
...
void CSFont::RenderShader(ID3D11DeviceContext *devicecontext,int indexCount)
{
