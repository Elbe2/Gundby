...
// SVerticalBlur.cpp
// Die Klasse für einen Shader mit vertikalen Blur(verticalblur.vs+verticalblur.ps)
// ------------------------------------------------------------------

#include "src\Graphics\Shader\SVerticalBlur.h"

CSVerticalBlur::CSVerticalBlur(void)
{
	m_VertexShader=NULL;
	m_PixelShader=NULL;
	m_Layout=NULL;
...
bool CSVerticalBlur::Init(ID3D11Device *device,HWND hWnd)
{
	if(!InitShader(device,hWnd,L"Shaders/blur/verticalblur.vs",L"Shaders/blur/verticalblur.ps"))
		return false;
	return true;
}

void CSVerticalBlur::Shutdown(void)
{
	ShutdownShader();
}

bool CSVerticalBlur::Render(ID3D11DeviceContext *devicecontext,int indexCount,XMMATRIX worldmat,XMMATRIX viewmat,XMMATRIX projmat,ID3D11ShaderResourceView **texture,float screenheight)
{
	if(!SetShaderParameters(devicecontext,worldmat,viewmat,projmat,texture,screenheight))
		return false;
	RenderShader(devicecontext,indexCount);
	return true;
}

bool CSVerticalBlur::InitShader(ID3D11Device *device,HWND hWnd,WCHAR *vsFilename,WCHAR *psFilename)
{
	...
	if(FAILED(D3DCompileFromFile(vsFilename,NULL,NULL,"VerticalBlurVertexShader","vs_5_0",D3D10_SHADER_ENABLE_STRICTNESS,0,&vertexShaderBuffer,&errorMessage)))
	{
		if(errorMessage)
	...
	if(FAILED(D3DCompileFromFile(psFilename,NULL,NULL,"VerticalBlurPixelShader","ps_5_0",D3D10_SHADER_ENABLE_STRICTNESS,0,&pixelShaderBuffer,&errorMessage)))
	{
		if(errorMessage)
...
void CSVerticalBlur::ShutdownShader(void)
{
	if(m_SampleState)
	{
...
bool CSVerticalBlur::SetShaderParameters(ID3D11DeviceContext *devicecontext,XMMATRIX worldmat,XMMATRIX viewmat,XMMATRIX projmat,ID3D11ShaderResourceView **texture,float screenheight)
{
...
void CSVerticalBlur::RenderShader(ID3D11DeviceContext *devicecontext,int indexCount)
{
